...
int main() {
    {
        ...
        for (const auto &p: request.params) {
            ...
            if (p.first == "grant_type") {
                grantType = p.second;
            }
            if (p.first == "refresh_token") {
                refreshToken = p.second;
            }
        ...
        if (grantType != "authorization_code") {
            std::cerr << "Wrong grant type\n";
            return 1;
        }
        if (!refreshToken.empty()) {
            std::cerr << "Not expected refresh token\n";
            return 1;
        }
    ...
    {
        OidcTokenRequest requestGenerator{"https://testing.radiotube.org", "client-id", jwk,
                                          {"scope1", "scope2"}, "rt"};
        auto request = requestGenerator.GetTokenRequest();
        JwkPemRsaKey jwkobj{};
        jwkobj.FromJwk(jwk);
        ...
        if (refreshToken.empty()) {
            std::cerr << "Expected refresh token\n";
            return 1;
        }
        Rs256 rs256{jwkobj.ToVerificationKey()};
        {
